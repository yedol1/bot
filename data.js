const authorData = {
  강인해: { name: "강인해", color: "purple" },
  양진영: { name: "양진영", color: "blue" },
  박경호: { name: "박경호", color: "gray" },
  이광렬: { name: "이광렬", color: "orange" },
  정태현: { name: "정태현", color: "green" },
  이종원: { name: "이종원", color: "red" },
  조문근: { name: "조문근", color: "brown" },
};

const bugData = {
  디자인: { name: "디자인", color: "purple" },
  기능: { name: "기능", color: "yellow" },
  기타: { name: "기타", color: "green" },
};

const developer = {
  U066HPC6GAG: "이광렬",
  U07K1MGDRQF: "양진영",
  U05CWRU2K60: "정태현",
  U06RYHLNK2S: "이종원",
  U06S7LJ7L8L: "조문근",
};
const member = {
  U066HPC6GAG: { name: "이광렬", color: "orange" },
  U07K1MGDRQF: { name: "양진영", color: "blue" },
  U05CWRU2K60: { name: "정태현", color: "green" },
  U042VFX4WMB: { name: "강인해", color: "purple" },
  U0430TP80SG: { name: "박경호", color: "gray" },
  U06RYHLNK2S: { name: "이종원", color: "red" },
  U06S7LJ7L8L: { name: "조문근", color: "brown" },
};

module.exports = { authorData, bugData, member, developer };
